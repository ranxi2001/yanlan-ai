...
test("default MiMo upload bounds reject unsafe browser decoding before transcription", () => {
  ...
  assert.match(mimoUploadLimitMessage({ protocol: "mimo-chat", size: MAX_MIMO_UPLOAD_BYTES + 1, duration: 1 }), /128 MiB/);
  ...
  for (const duration of [0, null, undefined, Number.NaN, Number.POSITIVE_INFINITY]) {
    assert.equal(mimoUploadLimitMessage({ protocol: "mimo-chat", size: MAX_MIMO_FALLBACK_BYTES, duration }), "");
    assert.match(mimoUploadLimitMessage({ protocol: "mimo-chat", size: MAX_MIMO_FALLBACK_BYTES + 1, duration }), /无法读取音频时长.*40 MiB/);
  }
